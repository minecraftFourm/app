...
export const handleNewPost = async (req: Req) => {
	const {
		body: { title, content, category: categoryId },
		user: {
			id,
			role: { canPostAdmin, canCreatePost, isAdmin },
		},
	} = req;

	// * If user does not have permission to create new post.
	if (!(canCreatePost || isAdmin || canPostAdmin))
	...
	if (!title)
		throw new CustomError("Post title is missing", StatusCodes.BAD_REQUEST);
	if (!content)
		throw new CustomError(
...
export const handleEditPost = async (req: Req) => {
	const {
		...
		user: {
			id: ownerId,
			role: { canEditPost, canEditOtherPost, isAdmin },
		},
	} = req;
	...
	if (
		!(
			(post?.ownerId === ownerId && canEditPost) ||
			canEditOtherPost ||
			isAdmin
		)
	) {
...
export const handleDeletePost = async (req: Req) => {
	const {
		...
		user: {
			id: ownerId,
			role: { canDeletePost, canDeleteOtherPost, isAdmin },
		},
	} = req;
    ...
    */
	...
	if (
		!(
			(post?.ownerId === ownerId && canDeletePost) ||
			canDeleteOtherPost ||
			isAdmin
		)
	) {
	...
	try {
		...
		if (error.code === "P2025")
			...
			throw new CustomError(
				"Something went wrong while trying to delete this post.",
