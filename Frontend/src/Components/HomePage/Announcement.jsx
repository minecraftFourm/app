...
const Announcement = ({...props}) => {
  ...
  const Component = () => {
    return Items.map(item => {
      const { id, title, updated, content, owner: { username }, comments, ownerId } = item

      return (
        <div className="w-full border border-gray-400 bg-white rounded-sm overflow-hidden" key={id}>
