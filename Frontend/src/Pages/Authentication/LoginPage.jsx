...
import { EMAIL_PATTERN, PASSWORD_PATTERN, PASSWORD_REQUIREMENT } from "../../config";
...
const LoginPage = () => {
    ...
    useEffect(() => {
        updateOptions({ property: loginDetails.email, value: 'validEmail', PATTERN: EMAIL_PATTERN })
        updateOptions({ property: loginDetails.password, value: 'validPassword', PATTERN: PASSWORD_PATTERN })
    }, [debounceValue])
    ...
    useEffect(() => {
        ((options.validEmail.valid) && (options.validPassword)) ? setFormSubmittable(true) : setFormSubmittable(false) 
    })
                ...
                <div className="border-solid border-[1px] drop-shadow-md overflow-hidden border-slate-300 mt-20 rounded-md max-w-md m-auto">
                    ...
                    <form className="flex flex-col p-4 gap-4 bg-white">
                        <div className="flex flex-col">
                            ...
                            <span className={`text-red-600 text-sm font-normal w-full mx-1 ${options.validEmail.showErr ? 'block' : 'hidden'}`}>Kindly please provide a valid email.</span>
                        </div>
                        <div className="flex flex-col">
