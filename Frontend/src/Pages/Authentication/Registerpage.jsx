...
const Registerpage = () => {
	...
	useEffect(() => {
		updateOptions({
			property: registerDetails.email,
			value: "validEmail",
			PATTERN: EMAIL_PATTERN,
		});
		updateOptions({
			property: registerDetails.username,
			value: "validUsername",
			PATTERN: USERNAME_PATTERN,
		});
		updateOptions({
			property: registerDetails.password,
			value: "validPassword",
	...
	useEffect(() => {
		...
		options.validUsername.valid &&
		options.validPassword.valid
				...
				<div className="border-solid border-[1px] drop-shadow-md overflow-hidden border-slate-300 mt-20 rounded-md max-w-md m-auto">
					...
					<form className="flex flex-col p-4 gap-2 bg-white">
						<div className="flex flex-col">
							...
							<input
								className="bg-indigo-100 rounded-md p-2 border-[1px] outline-slate-500"
