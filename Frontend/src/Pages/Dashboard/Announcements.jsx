...
const Announcements = () => {
	...
	useEffect(() => {
		(async () => {
			...
			const { data, response } = await CustomFetch({
				url: `post?category=${ANNOUNCEMENT_CATEGORY_NAME}&limit=50`,
				returnResponse: true,
			});
			setAnnouncements(() => {
				return {
	...
	const announcementComponent = announcements.data.map((item) => {
		return (
			<div
				key={item.id}
				className="border border-indigo-500 m-1 rounded-md px-2 py-1 w-full">
				...
				<footer className="flex w-full justify-between items-center">
					<p className="font-normal text-sm text-gray-500 cursor-default">
						Posted by{" "}
					...
					<div className="flex gap-2 items-center">
						<p className="font-light text-sm text-gray-500 cursor-default">
							{format(item.updated)}
						</p>
						<p>|</p>
						<p className="font-light text-sm text-gray-500 cursor-default">
			...
			<header className="mb-2 py-2 px-4 bg-white drop-shadow-md">
				<div className="w-full items-center gap-2 justify-between flex">
					<input
						type="text"
						placeholder="Search..."
						className="rounded-md px-2 py-1 border-indigo-500 w-full outline-none"
					/>
					<Link
						to="../newAnnouncement"
						className=""
				...
				<p className="text-sm">
					Showing {announcements.count} announcements.
				</p>
			</header>
