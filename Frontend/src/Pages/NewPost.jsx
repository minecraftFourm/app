    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                setCategoryData(() => {
                    let isAdmin = (User && User.role && User.role.isAdmin) ? true : false 
                    /* 
  ...
  return (
    <div className='bg-[#1B263B] pb-16'>
        ...
        <div className='bg-white px-4 mx-6 md:mx-2 sm:mx-0 sm:px-2 mt-16 py-4'>
            ...
            { ( err && !isLoading ) && 
                <p className='text-lg text-gray-400 text-center'>An error has occured while trying to fetch data from the server.</p>
            }
            { (!isLoading && !err ) && 
                <>
                    <h1 className='font-bold text-indigo-700 text-4xl mb-4 sm:text-2xl'>Create a new post: </h1>
                
                    <form onSubmit={handleSubmit} className="flex flex-col gap-2">
                        <input type="text" className="px-2 py-1 outline-none border border-gray-400 rounded-sm w-full" placeholder='Title...' size="200" onChange={e => updateState({ title: e.currentTarget.value })} value={inputState.title} />
                        <Select 
                            options={categoryData} 
                            className="z-10 mb-2 basic-single" 
                            classNamePrefix="select"
                            placeholder={categoryData ? "Category..." : "There are no categories available..." }
                            defaultValue={ (categoryData && defaultCategory != null) ? categoryData[defaultCategory] : null }
                            ...
                            onChange={e => {
                                    if (e === null) {
                                        updateState({ category: null, categoryId: null });
                                    }
                                    else {
